...
/*
When the objloader class detects that a face from a mesh corresponds to materials named 'collision' the face is not rendered but is stored as a
collision plane. These collision planes are used to in collision detection
*/
...
collisionplane::collisionplane(float n1, float n2, float n3, float a, float b, float c, float d, float e, float f, float g, float h, float i){
	p[0].change(a, b, c);
	p[1].change(d, e, f);
	p[2].change(g, h, i);
	normal.change(n1, n2, n3);
